...
> **Terceira atualização — correções após revisão do repositório:**
> Analisei todo o conteúdo do seu repositório (`Provas-Laura`) e encontrei
> alguns problemas que já estavam quebrando o app silenciosamente:
>
> 1. **`questions.json` estava com JSON inválido** (faltava uma chave `}`
>    de fechamento no final) — isso fazia o app inteiro falhar ao carregar
>    as questões. Corrigido.
> 2. **9 questões de matemática (tabuada do 2) tinham alternativas
>    repetidas** (ex.: opções `[6, 3, 8, 8]`) — corrigido, mantendo a
>    resposta certa no mesmo lugar.
> 3. **As imagens dos ícones (`icons/icon-192.png` e `icon-512.png`)
>    nunca foram enviadas ao repositório.** Isso é mais grave do que
>    parece: o Service Worker usa `cache.addAll()`, que **falha por
>    completo** se qualquer arquivo da lista não existir — ou seja, o
>    app **nunca conseguiu funcionar offline**, mesmo antes desta
>    conversa. Também deixei o Service Worker mais resiliente (cacheia
>    cada arquivo individualmente agora, então um arquivo faltando não
>    derruba os outros) e incluí os ícones de novo neste pacote.
> 4. **O carregamento do EmailJS tinha voltado à versão antiga** (sem a
>    segunda fonte de CDN nem a espera tolerante) — restaurado.
> 5. Atualizei a lista de disciplinas mostrada na tela inicial: seu banco
>    cresceu para **1000 questões objetivas + 38 discursivas em 11
>    disciplinas** (adicionou Artes, Tecnologia e Cidadania e Ética —
>    parabéns pela expansão!). Como são 11 disciplinas para 10 vagas por
>    prova, cada prova sorteia 10 das 11, deixando uma de fora por vez.
>
> Testei tudo antes de entregar: validação de JSON, integridade de cada
> uma das 1000 questões, geração de várias provas, fluxo completo
> (objetivas + discursiva), envio de e-mail (sucesso, SDK bloqueado,
> chaves erradas) e a instalação do Service Worker mesmo com um arquivo
> faltando.
