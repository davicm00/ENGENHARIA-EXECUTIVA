...
async function showResults(){
  ...
  if (emailStatusEl) {
    if (result.sent) {
      ...
      emailStatusEl.textContent = 'ℹ️ As chaves do EmailJS ainda não foram preenchidas em app.js.';
    } else if (result.reason === 'sdk_unavailable') {
      emailStatusEl.innerHTML = '⚠️ Não foi possível carregar o serviço de envio de e-mail agora. Isso costuma acontecer quando a rede/wifi bloqueia o endereço do CDN (comum em wifis com controle parental). O resultado foi salvo. <button id="btnRetryEmail" class="btn btn-secondary" style="margin-top:10px;padding:10px;font-size:13px;">Tentar enviar de novo</button>';
      const retryBtn = document.getElementById('btnRetryEmail');
      if (retryBtn) retryBtn.addEventListener('click', retrySendCurrentReport);
    } else if (result.reason === 'offline') {
      emailStatusEl.textContent = '📶 Sem internet no momento — o e-mail será enviado automaticamente assim que reconectar.';
    } else {
      // result.reason === 'send_error': está online, mas o EmailJS recusou/falhou o envio
      // (ex.: Service ID/Template ID/Public Key errados). Mostramos o detalhe técnico
      // para facilitar o diagnóstico, e o resultado fica salvo para tentar de novo.
      emailStatusEl.innerHTML = '⚠️ Não foi possível enviar o e-mail agora (não é falta de internet — verifique as chaves do EmailJS em app.js). O resultado foi salvo e será reenviado automaticamente.' +
        (result.detail ? '<br><span style="font-size:11px;opacity:0.8;">Detalhe técnico: ' + escapeHtml(String(result.detail)) + '</span>' : '') +
        ' <button id="btnRetryEmail" class="btn btn-secondary" style="margin-top:10px;padding:10px;font-size:13px;">Tentar enviar de novo</button>';
      const retryBtn2 = document.getElementById('btnRetryEmail');
      if (retryBtn2) retryBtn2.addEventListener('click', retrySendCurrentReport);
    }
  }
...
async function retrySendCurrentReport(){
  emailStatusEl.textContent = '📤 Tentando enviar novamente...';
  await flushQueuedReports();
  updatePendingQueueNotice();
  const stillQueued = getQueuedReports().some(r => r.name === (studentName || 'Aluna'));
  emailStatusEl.textContent = stillQueued
    ? '⚠️ Ainda não foi possível enviar. O resultado continua salvo e será reenviado automaticamente.'
...
function updatePendingQueueNotice(){
  const el = document.getElementById('pendingQueueNotice');
  if (!el) return;
  const queue = getQueuedReports();
  el.textContent = queue.length > 0
    ? ('⏳ ' + queue.length + ' resultado(s) ainda aguardando envio por e-mail.')
    : '';
}

// Inicialização: carrega as questões e tenta reenviar relatórios pendentes
showScreen('start');
loadQuestions().then(updatePendingQueueNotice);
updatePendingQueueNotice();
window.addEventListener('online', () => { flushQueuedReports().then(updatePendingQueueNotice); });
if (navigator.onLine) flushQueuedReports().then(updatePendingQueueNotice);
